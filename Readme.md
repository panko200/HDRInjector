...
本プラグインは HDR 動画の入力処理を GPU 中心の経路へ置き換えるため、環境によっては初回再生時に描画が重くなる場合があります。
...
v1.1.0  
HDR機能のON/OFFの切り替え設定を追加(再起動時に適用)  
HDR動画を読み込ませたとき、画面の複製やグループ制御をかけると、HDR動画が初回1フレーム目が真っ暗になる問題の修正  
グループ制御境界を跨いだ逆方向シークをすると、順方向シークするまでHDR動画が真っ暗になる問題の修正
